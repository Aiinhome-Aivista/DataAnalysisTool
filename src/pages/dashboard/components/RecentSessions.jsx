...
function RecentSessions() {
    ...
    const [loadingSession, setLoadingSession] = useState(null); // To track which session is loading
    ...
    const handleViewClick = async (rowData) => {
        ...
        try {
            ...
            updateSessionData(sessionName, { dataTypes, patterns, relationships: responseData.relationships || [], insights: result?.insights?.insights || [], graphUrl: result?.graph_url || "" });
        ...
        <div className="p-6 min-w-0 border border-slate-700 rounded-lg">
            <div className="flex justify-between items-center mb-4">
                ...
                <button
                    ...
                    className={`p-2 rounded-md transition-colors ${loading ? 'text-slate-500 cursor-not-allowed' : 'text-slate-400 hover:bg-slate-700 hover:text-white'}`} disabled={loading}>
                        ...
                        Array.from({ length: 5 }).map((_, i) => (
                            <div key={i} className="flex items-center justify-between p-3 mb-4">
                                <div className="flex-1">
                                    <Skeleton width="60%" height="1.25rem" className="mb-2 !bg-slate-700" />
                                    <Skeleton width="40%" height="0.875rem" className="!bg-slate-700" />
                                </div>
                                <Skeleton shape="circle" size="2rem" className="ml-4 !bg-slate-700" />
                            </div>
                        ))
                    ) : recentSessions && recentSessions.length > 0 ? (
                        recentSessions.slice(0, 5).map((session) => {
                            const isLoading = loadingSession === session.SESSION_NAME;
                            const isCompleted = session.SESSION_STATUS?.toLowerCase() === "completed" || session.SESSION_STATUS?.toLowerCase() === "success";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex gap-2">
                                        <button disabled={isDisabled} className={`p-2 rounded-md transition-colors ${isDisabled ? "bg-gray-500 cursor-not-allowed" : "bg-[#795eff] hover:bg-[#6a4be8] cursor-pointer"}`} onClick={() => handleViewClick(session)}>
                                            {isLoading ? (
